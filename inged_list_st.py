...
if authentication_status:
    ...
    if len(yt_link) > 2:
        ...
        if st.button("Submit Meal Name") == True and len(meal_id) > 1 and len(meal_name) > 1 and len(meal_description) and len(meal_name_hindi) > 1 or st.session_state.meal_submitted_flag == 1:
            # Uplad meal data
            ...
            upload_meal_data = {
                ...
                u'youtube_link': meal_url,
                u'nameAsArray': name_list


            }
            db.collection(u'recipes').document(meal_id).set(upload_meal_data)
                ...
                if st.session_state.ingred_flags[i] == 0:
                    ...
                    if check:
                        ...
                        if ingred_dropdown != "None" and ingred_dropdown != ".":
                            ...
                            if submit_ingred:
                                db.collection(u'recipes').document(meal_id).collection(
                        ...
                        if ingred_dropdown == "None":
                            ...
                            english_name = st.text_input(
                                'english name'+str(i), '')
                            hindi_name = st.text_input('Hindi name'+str(i), '')
                            image_url = st.text_input('Image_url'+str(i), '')
